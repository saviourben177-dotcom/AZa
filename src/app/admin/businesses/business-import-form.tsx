...
// src/app/admin/businesses/business-import-form.tsx
//
// Two-step flow: parse+validate (no DB writes) -> review -> commit.
// Invalid rows are shown with their specific error and excluded from commit;
...
export function BusinessImportForm({ defaultOwnerId }: { defaultOwnerId: string }) {
  const [preview, setPreview] = useState<ImportPreview<BusinessInput> | null>(null);
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const result = await previewBusinessImport(text, format);
      setPreview(result);
      setStatus("idle");
    } catch (err) {
      setStatus("error");
  ...
  async function handleCommit() {
    ...
    try {
      const validRows = preview.rows
        ...
        .map((r) => r.data);
      await commitBusinessImport(validRows, defaultOwnerId);
      setStatus("done");
      setPreview(null);
    } catch (err) {
      setStatus("error");
