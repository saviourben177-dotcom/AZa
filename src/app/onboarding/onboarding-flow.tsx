...
export default function OnboardingFlow() {
  ...
  function toggleStatusValue(value: EmploymentStatusOption) {
    setData((d) => {
      const arr = d.status ?? [];
  ...
  function toggleLearningContextValue(value: string) {
    setData((d) => {
      const arr = d.learning_context ?? [];
      const has = arr.includes(value);
      const updated = has ? arr.filter((v) => v !== value) : [...arr, value];
      return { ...d, learning_context: updated };
    });
  }
          ...
          <StepShell title="What do you do?" subtitle="Select all that apply — we'll ask the right follow-up for each.">
            <div className="mt-6 space-y-2.5">
              {STATUS_OPTIONS.map((opt) => (
                <CheckRow key={opt.value} label={opt.label} checked={(data.status ?? []).includes(opt.value)} onClick={() => toggleStatusValue(opt.value)} />
              ))}
          ...
          <StepShell title="Are you currently learning?" subtitle="Select all that apply.">
            <div className="mt-6 space-y-2.5">
              {LEARNING_CONTEXTS.map((opt) => (
                <CheckRow key={opt.value} label={opt.label} checked={(data.learning_context ?? []).includes(opt.value)} onClick={() => toggleLearningContextValue(opt.value)} />
              ))}
            </div>
          </StepShell>
        )}
