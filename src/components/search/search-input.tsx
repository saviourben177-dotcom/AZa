      ...
      <div className="flex items-center gap-2">
        ...
        <form onSubmit={submit} className="relative flex-1">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="absolute left-4 top-1/2 -translate-y-1/2">
            <circle cx="11" cy="11" r="7" stroke="rgb(var(--ink) / 0.5)" strokeWidth="1.8" />
            <path d="M20 20l-3.5-3.5" stroke="rgb(var(--ink) / 0.5)" strokeWidth="1.8" strokeLinecap="round" />
          </svg>
          <input
            autoFocus
            value={value}
            ...
            className="h-11 w-full rounded-pill bg-paper-dim pl-11 pr-4 text-[14px] text-ink placeholder:text-ink/35 focus:outline-none"
          />
        </form>
      </div>
      ...
      {recent.length > 0 && (
        <div className="mt-5">
          <div className="flex items-center justify-between">
            <h2 className="text-[13px] font-semibold uppercase tracking-wide text-ink/45">Recent searches</h2>
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {recent.map((term) => (
              <button
                ...
                className="rounded-pill bg-paper-dim px-3.5 py-2 text-[12.5px] font-semibold text-ink/55"
              >
                {term}
              </button>
            ))}
