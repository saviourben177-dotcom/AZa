...
1. Apply the migration (already applied directly to the Supabase project as
   of this note — confirm it's present in your local `supabase/migrations/`
   folder too, or pull it down with the Supabase CLI so your migration
   history stays in sync).
2. Set your own account's `profiles.role` to `'admin'`:
   ```sql
...
- **Known v1 limitation**: because `ideas` INSERT RLS is owner-only
  (unchanged from the original schema, on purpose), seeded teams must be
  owned by your own admin account, not by `@Aza` directly. If you want
  `@Aza`-owned teams, that needs one additional narrow INSERT policy — not
  added in this version; ask if you hit this wall.
